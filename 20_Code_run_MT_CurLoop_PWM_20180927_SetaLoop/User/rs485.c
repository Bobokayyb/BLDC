...
void USART3_IRQHandler(void)
{
	...
	{	 
		...
		yyb_test_RS485 =USART_ReceiveData(USART3); 	//读取接收到的数据
	}  											 
} 
#endif										 
//初始化IO 串口2
...
{  
  ...
  USART_InitTypeDef USART_InitStructure;
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;				 //485的RS引脚
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOB, &GPIO_InitStructure);
 
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;	//发送引脚
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;	//复用推挽   复用UART3引脚
  GPIO_Init(GPIOB, &GPIO_InitStructure);
...
void RS485_Send_Data(void)
{
	...
	USART_SendData(USART3,yyb_test_RS485);	 
